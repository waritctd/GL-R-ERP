...
- **Backend:** session auth via `SessionSecurityFilter`. `SecurityConfig` is **default-deny** — `anyRequest().authenticated()`, with seven anonymous exceptions as of the self-service forgot-password change (2026-09-22): OPTIONS preflight, `POST /api/auth/login`, `POST /api/auth/forgot-password`, `POST /api/auth/reset-password`, `POST /api/attendance/punch`, `GET /actuator/health(/**)`, and `GET /api/public/brand/**`. This bullet has been wrong before (claimed `permitAll` and "no Actuator/OpenAPI" until 2026-08-08; claimed "four" when it was actually five just before this edit) — read the file rather than trusting a count here. **Role checks live in the controllers, not the filter chain**, so "authenticated" is the only guarantee `SecurityConfig` gives you.
