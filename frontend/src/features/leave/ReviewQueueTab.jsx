...
//
// TODO(A4): a persistent detail panel (instead of the expand-row disclosure below) plus the
// medical-certificate attachment download belong here -- out of scope for this phase, which is a
// pure relocation of the pre-A1 table's existing approve/reject/cancel behaviour.
...
export function ReviewQueueTab({ user, showToast }) {
    ...
    renderActions: (request) => {
      ...
      return (
        <>
          ...
          {cancellable ? (
            <Button type="button" variant="icon" title="ยกเลิก" aria-label="ยกเลิก" disabled={saving} onClick={() => requestCancel(request.id)}>
              <Icon name="close" size={14} />
            </Button>
          ) : null}
        </>
  ...
  }), [expandedId, saving, user, canReviewAll]);
      ...
      <>
        ...
        {reviewable || cancellable ? (
          <div className="mt-1 flex gap-2">
            ...
            {cancellable ? (
              <Button type="button" variant="secondary" className="min-h-11 flex-1" disabled={saving} onClick={() => requestCancel(request.id)}>
                <Icon name="close" size={14} />
                ยกเลิก
              </Button>
            ) : null}
