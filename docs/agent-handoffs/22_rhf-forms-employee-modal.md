...
- `frontend/src/features/employees/EmployeeFormModal.jsx`:
  ...
  - **Division→department reset (parity fix, post-review):** the original `update()` cleared `departmentTh` to `''` whenever `divisionId` changed, keeping the department consistent with the selected ฝ่าย. The first migration draft dropped this, so after a division change a stale `departmentTh` could flow into the submitted payload. Restored by adding `setValue` to the `useForm` destructure and wiring `register('divisionId', { onChange: () => setValue('departmentTh', '') })` on the division `<select>`. Only the `departmentTh` reset was needed — `divisionTh` is recomputed from `divisionId` at submit time.
...
- `frontend/src/features/employees/EmployeeFormModal.test.jsx` (new): five tests —
  ...
  4. division→department reset (parity fix): seeds an edit-mode `employee` with `divisionId: '17'` / `departmentTh: 'สรรหาบุคลากร'`, changes the division `<select>` to `'10'`, asserts the department input clears to `''`, and asserts the submitted payload carries `departmentTh: ''` (not the stale value) with `divisionId: '10'` / `divisionTh: 'AC-ฝ่ายบัญชี'`;
  5. edit-mode: seeds all `defaultValues` from a fixture `employee` object (including nested `currentAddress`/`emergencyContact`), asserts the inputs display the seeded values, and asserts an unmodified submit reproduces the same values in the payload (with `divisionTh` recomputed from the matched division, `salary` coerced through `Number(...)`). Uses a far-future `hireDate` (`2099-01-15`) to avoid any date-dependent flakiness (`hireDate` has no zod rule, but the fixture avoids look-alike-today values as a matter of test hygiene).
...
- Full frontend tests: pass. `npm test -- --run` → 14 files / 72 tests passed (includes the 5 new `EmployeeFormModal` tests; up from 67 tests in the Phase 3 Leave handoff, since 5 new tests were added and 0 were removed).
...
- Browser smoke: not run this session (no `frontend-mock` preview server was started); recommend the next reviewer do a quick manual pass (`Create employee` / `Edit employee` flows) before merge, though behavior is covered by the five automated tests plus close reading of the diff.
...
- The five new tests cover required/format/create-payload/edit-payload parity plus the division→department reset. They do not cover the "existing employee's division not in the known/derived list" fallback-merge branch of `divisionOptions` — that is unchanged logic (verbatim from the pre-migration `useMemo`), so risk is limited to whatever pre-existed.
