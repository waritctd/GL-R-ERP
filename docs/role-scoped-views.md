...
- `ค่าคอมมิชชัน` — **dropped from account's nav** (finalized Account design,
  owner-confirmed 2026-07-24). The invoice+commission step (`บันทึกใบกำกับ +
  ออกค่าคอม`) is the **last stage of the `งานการเงิน` money lifecycle** + the
  Overview worklist — `รับมัดจำ → รับชำระส่วนที่เหลือ → ยืนยันพร้อมปิดงาน →
  บันทึกใบกำกับ + ออกค่าคอม` — not a standalone menu item. The `งานการเงิน`
  commission row deep-links `/commissions?ticketId=NN` (the `CommissionPage.jsx`
  create-from-deal flow). The `/commissions` **route** stays reachable
  (`canViewCommissions` still includes account, for that deep-link); account has
  no commission *list* access — `GET /api/commissions` is `canListCommissionRecords`
  = sales/sales_manager/ceo only (enforced backend-side, CommissionService.list).
...
- `frontend/src/components/layout/AppShell.jsx` — `รายการดีล` gated on
  `canViewDealPipeline`; `ค่าคอมมิชชัน` hidden for account (folded into `งานการเงิน`);
...
### Employee (base, self-service) — shipped `feat/role-views-employee`

**Audience**: `role === 'employee'` and **not** a division manager. A plain,
non-managing employee — every other role (including `employee` who *is* a
division manager) keeps the existing `EmployeeDashboard` manager-mode
branch, which is out of scope for this slice. `warehouse`/`qc` also keep
`EmployeeDashboard` for now (same `canUseEmployeeExperience` data wiring,
no scoped warehouse/QC view yet — a separate later change).

**"Is this a division manager?" detection**: `!!user?.manager` — the exact
predicate `EmployeeDashboard`'s `dashboardMode()` and `AttendancePage`'s
`attendanceMode()` already use. Not re-derived; `App.jsx` computes
`isPlainEmployee = user?.role === 'employee' && !user?.manager` from it, so
all three surfaces (dashboard, attendance, landing) agree on who counts as a
manager.

**Nav**: unchanged. `AppShell.jsx`'s existing `show:` predicates already
scope a plain employee to แดชบอร์ด · เวลาทำงาน · คำขอ (OT/สวัสดิการ) · วันลา
only — no `canViewTickets`/`canViewEmployees`/`canManagePayroll`/etc. key
includes `'employee'`, so no sales or HR item was ever reachable. Verified,
not changed. (โปรไฟล์ของฉัน lives in the topbar `UserMenu`, not the sidebar —
pre-existing, unrelated to this slice.)

**Landing**: `frontend/src/features/dashboard/EmployeeSelfService.jsx`
(new), mounted at `/` for `isPlainEmployee`. Built around four blocks:

1. **Clock card** — today's เข้างาน time + เข้างานแล้ว/ยังไม่ลงเวลา status.
   **No self-punch action exists in the frontend today** — attendance is
   derived only from badge-scanner `.dat` imports
   (`AttendancePage.jsx`/`routes.js` have no clock-in/out POST route) — so
   the card is a link to `/attendance`, not a real punch mutation. Stated
   here rather than silently rendering a button that does nothing.
2. **Three tiles** — เวลาทำงานเดือนนี้ (days present + late count, from
   `api.attendance.daily({})`, own scope, current month), วันลาคงเหลือ
   (per-type balance from `api.leave.balances({})`, own scope), สลิปเงินเดือน
   (`dashboardSummary.latestPayrollPeriodId` + `api.payroll.downloadOwnPayslip`,
   same pattern `EmployeeDashboard` already used).
3. **คำขอของฉัน panel** — own leave + OT + profile-change requests merged
   and sorted by request date, each row showing its status and approval
   chain (ส่งแล้ว → หัวหน้าฝ่าย → CEO for OT; ส่งแล้ว → อนุมัติ for leave,
   single reviewer; ส่งแล้ว → HR อนุมัติ for profile-change requests), naming
   the approver once known. "+ ขอลา" / "+ ขอ OT" link into the existing
   create flows on `/leave` and `/employee-requests?tab=ot`.
4. No approval/management affordance anywhere on the page — it is
   self-service only, enforced by test (`EmployeeSelfService.test.jsx`
   asserts no อนุมัติ/ปฏิเสธ control is ever rendered).

**Data — all own-scoped reads, no backend change**:
- `api.attendance.daily({})` — own scope (mirrors `AttendancePage`'s
  self-view: `hasPermission(user.role, 'canViewAllAttendance')` is false and
  `user.manager` is false, so the backend/mock already narrows to self).
- `api.leave.balances({})`, `api.leave.list({})` — own scope (mock:
  `employeeId` defaults to the caller; `list()` filters to
  `item.employeeId === user.employeeId` when the caller isn't hr/ceo/manager).
- `api.overtime.list({})` — same own-scope shape as leave.
- `profileRequests` prop — the same `ownRequests`/`dashboardRequests` value
  `App.jsx` already computed for `EmployeeDashboard`'s employee mode;
  unchanged.
- `dashboardSummary.latestPayrollPeriodId` — **known mock gap, not invented
  data**: `mockApi.js`'s `dashboard.summary()` hardcodes this to `null`, so
  the สลิปเงินเดือน tile always renders its empty state under
  `VITE_USE_MOCKS=true`. That's the mock's real (empty) value passed
  through, not a fabricated number — the same gap `EmployeeDashboard`
  already had for its "My payslip" button.

**Backend**: none. **Authz**: no change — every read above already scopes to
the caller server-side; this slice only decides which frontend component
renders the already-correct data. Not applicable for a real-DB integration
test (no gate moved).

### Division Manager (non-sales) — shipped `feat/role-views-division-manager`

**Who:** role `employee` **and** the manager flag (`DivisionAccessPolicy.isManager`: position
contains "ผู้จัดการ", assistant managers included) — e.g. a ฝ่ายคลังสินค้า (warehouse) manager.
Detected by `isDivisionManager(user)` in `app/permissions.js`, narrowed to `role === 'employee'` so
`sales_manager`/`hr`/`ceo` (which have their own landings) are not swept in. This role sits outside
the sales-pipeline role → view-access matrix the other six sections above cover — it is not a
sales/CRM role and never had a pipeline.

**Why not the generic manager mode:** `EmployeeDashboard`'s `dashboardMode()` 'manager' branch is a
flat stat-card grid with no worklist and no self-service framing. A division manager's actual job
is the first step of the OT/leave approval chain for their division plus knowing their team's
attendance today — that needed a dedicated worklist, not another card grid.

**Approval model this Overview encodes** — OT/leave route `employee → division manager (step 1) →
CEO` in the reconciled model, but **the two workflows are not symmetric in this codebase today**:

- **Overtime** is genuinely 3-step: `SUBMITTED → MANAGER_APPROVED → APPROVED` (see
  `OvertimePanel.canCeoApprove`/`canManagerApprove`). A division manager's approval is step 1; a
  CEO approval is required after.
- **Leave** is single-step here: `SUBMITTED → APPROVED`, with no `MANAGER_APPROVED`/CEO stage (see
  `LeavePage.canReviewRequest` and `mockApi.canReviewLeave` — HR or the direct manager approves
  directly, no second reviewer). The manager's approval is terminal.

The worklist's approval-chain badge reflects this difference explicitly (`พนักงาน ✓ → คุณ (ตอนนี้) →
CEO` for OT, `พนักงาน ✓ → คุณ (ตอนนี้)` for leave) rather than forcing a uniform 3-step shape onto a
workflow that doesn't have one — a deliberate, code-verified choice, recorded here per CLAUDE.md.

Scoping is also asymmetric and is NOT re-derived client-side — the Overview trusts what
`api.overtime.list()`/`api.leave.list()` return for the caller and only drops the caller's own
submitted request (no self-approval): OT review scope is division-wide (`canReviewOvertime`: direct
report OR same division as a position-derived manager); leave review scope is direct-report FK only
(`canReviewLeave`: no division fallback).

**Landing:** `frontend/src/features/dashboard/DivisionManagerOverview.jsx`, mounted at `/` in
`App.jsx` when `isDivisionManager(user)` is true (checked before the plain-employee
`isPlainEmployee` branch — see §"Landing branch" note above). Built around:

- **Pulse** (`StatGrid`): รออนุมัติ OT (team, amber) · รออนุมัติลา (team, amber) · มาสายวันนี้
  (team) · ทีมทั้งหมด (division headcount, from `dashboardSummary.headcount.active` — the same
  field `EmployeeDashboard`'s manager mode already used).
- **คำขอที่รอคุณอนุมัติ worklist**: OT + leave rows needing this manager's action, each with
  employee name, type/detail, the chain badge above, and an **อนุมัติ** button that deep-links —
  never inline-mutates. OT rows → `/employee-requests?tab=ot` (`OvertimePanel`); leave rows →
  `/leave` (`LeavePage`). **Known limitation, recorded rather than silently accepted**: neither page
  reads a request-id/employee-id query param to pre-filter to the single row that triggered the
  deep link — the CTA opens the correct page + tab, not the exact row. Out of scope for a
  frontend-only Overview slice.
- **การลงเวลาทีมวันนี้** (right rail): มาแล้ว/มาสาย/ยังไม่ออก counts + attention-first rows
  (late/missing-checkout sorted first), from `api.attendance.daily({from: today, to: today})` —
  already division-scoped server-side for a manager caller (`mockAttendanceScope`).
- **ของฉัน (self-service)** (right rail): ลงเวลา / ขอลา / ขอ OT quick actions — the manager's own
  self-service, since they are an employee too. Routes to the same `/attendance`, `/leave`,
  `/employee-requests?tab=ot` pages (which already combine "submit my own" + "review my team's" in
  one page, same pattern every other manager-shaped role in this codebase already uses).
- No sales surface anywhere on this page or its nav — this role never had one.

**Nav (`AppShell.jsx` / `Sidebar.jsx`):** new `team` nav group (label "ทีมของฉัน"), shown only for
`isDivisionManager(user)`: **การอนุมัติ OT** → `/employee-requests` (same route as the existing
self-service `คำขอ` item), **การอนุมัติวันลา** → `/leave` (same route as the existing self-service
`วันลา` item), **ทีมในฝ่าย** → `/attendance` (same route as the existing self-service `เวลาทำงาน`
item).

**Deliberate deviation, recorded**: the task asked for a distinct `ทีมในฝ่าย` (roster) nav entry.
There is no accessible roster page for this role — `canViewEmployees` (`/employees`) is HR-only in
`ROLE_PERMISSIONS`, and granting it would be an authorization change this frontend-only task does
not request and CLAUDE.md does not allow as a side effect. `/attendance` already gives a division
manager a per-employee, division-scoped view of their team (names + today's status), so it stands
in for "roster" rather than inventing a new page or loosening a permission.

The existing `self`-group entries (`เวลาทำงาน`, `คำขอ`, `วันลา`) are **unchanged** and still show for
everyone, including this role — they are the manager's own self-service, identical in every other
role. The three `team`-group entries above intentionally point at the same three routes, framed for
the approval/team angle; this is accepted duplication (same URL, two framings) rather than a
functional gap, since neither route has a "team-only" vs "self-only" variant to split into.

**Backend**: none. **Authz**: no change beyond the new client-side `isDivisionManager` helper —
every read above already scopes to the caller server-side; this slice only decides which frontend
component renders the already-correct data and which nav items appear.

**Files:**
- `frontend/src/features/dashboard/DivisionManagerOverview.jsx` (new) — the landing.
- `frontend/src/features/dashboard/DivisionManagerOverview.test.jsx` (new).
- `frontend/src/app/permissions.js` — added `isDivisionManager(user)`.
- `frontend/src/app/permissions.test.js` — tests for `isDivisionManager`.
- `frontend/src/App.jsx` — lazy-imports `DivisionManagerOverview`; `/` route branches on
  `isDivisionManager(user)`, checked before the `isPlainEmployee` fallback.
- `frontend/src/App.test.jsx` — route-branch integration tests + mock
  `api.overtime/leave/attendance`.
- `frontend/src/components/layout/Sidebar.jsx` — added the `team` nav group.
- `frontend/src/components/layout/AppShell.jsx` — added the three `team`-group nav items, gated on
  `isDivisionManager(user)`.
- `frontend/src/api/queryKeys.js` — `attendanceDaily(from, to)` key factory (shared with the
  Employee section's `EmployeeSelfService`, see above).
...
| 2026-07-24 | Account | `fix/account-nav-drop-commissions` | **Finalized Account design (owner-confirmed):** `ค่าคอมมิชชัน` is NOT a nav item — the invoice+commission step (`บันทึกใบกำกับ + ออกค่าคอม`) is the last stage of the `งานการเงิน` money lifecycle + Overview worklist (`รับมัดจำ → รับชำระส่วนที่เหลือ → ยืนยันพร้อมปิดงาน → บันทึกใบกำกับ + ออกค่าคอม`). Reverts a 2026-07-23 integration change that had restored the nav item on a mistaken "parity" read. `/commissions` route stays reachable (`canViewCommissions` incl. account) for the worklist's create-from-deal deep-link; the list API is `canListCommissionRecords` = sales/sales_manager/ceo only (now enforced backend-side, PR #292). Frontend-only. |
