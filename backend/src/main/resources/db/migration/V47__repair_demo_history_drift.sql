-- Forward-only compatibility for the hosted Render demo database.
--
-- That database was created while early migrations were still being renumbered. It can therefore
-- have a schema history row for old V11 while current source resolves V11.1/V11.2, plus checksum
-- drift on edited historical SQL. This migration makes the schema changes from V11.1/V11.2 safe
-- even when those versions are below the installed rank and cannot be applied normally.
...
CREATE TABLE IF NOT EXISTS sales.invoice_details (
...
CREATE TABLE IF NOT EXISTS sales.commission_record (
...
CREATE TABLE IF NOT EXISTS sales.tier_config (
...
CREATE INDEX IF NOT EXISTS idx_invoice_date ON sales.invoice_details(invoice_date DESC);
CREATE INDEX IF NOT EXISTS idx_commission_sales_rep ON sales.commission_record(sales_rep_id, payroll_month DESC);
CREATE INDEX IF NOT EXISTS idx_commission_payroll ON sales.commission_record(payroll_month, status);
CREATE INDEX IF NOT EXISTS idx_commission_ticket ON sales.commission_record(source_ticket_id);
CREATE INDEX IF NOT EXISTS idx_commission_invoice ON sales.commission_record(invoice_id);
