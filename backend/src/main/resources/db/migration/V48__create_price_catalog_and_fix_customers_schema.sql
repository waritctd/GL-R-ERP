-- V48: Forward-only recovery for the hosted demo DB.
--
-- V40 (catalog_import_schema) was never applied on the Supabase demo DB because version 40
-- was already occupied by an earlier migration (widen_ticket_payment_status). Similarly V41/V42
-- were skipped. This migration creates the price_catalog tables directly in the price_catalog
-- schema (idempotent with IF NOT EXISTS) and moves customer/contact/project to customers schema.

-- ── 1. price_catalog.factories ───────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS price_catalog.factories (
...
-- ── 2. price_catalog.import_profiles ─────────────────────────────────────────
CREATE TABLE IF NOT EXISTS price_catalog.import_profiles (
    profile_id  BIGSERIAL PRIMARY KEY,
    factory_id  BIGINT NOT NULL REFERENCES price_catalog.factories(factory_id),
    config      JSONB NOT NULL,
    updated_by  BIGINT,
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT uq_profile_factory UNIQUE (factory_id)
);

-- ── 3. price_catalog.price_list_versions ─────────────────────────────────────
CREATE TABLE IF NOT EXISTS price_catalog.price_list_versions (
...
CREATE INDEX IF NOT EXISTS idx_plv_factory
    ON price_catalog.price_list_versions(factory_id, status);

-- ── 4. price_catalog.product_prices ──────────────────────────────────────────
CREATE TABLE IF NOT EXISTS price_catalog.product_prices (
    price_id       BIGSERIAL PRIMARY KEY,
    factory_id     BIGINT NOT NULL REFERENCES price_catalog.factories(factory_id),
    version_id     BIGINT NOT NULL REFERENCES price_catalog.price_list_versions(version_id),
    product_code   TEXT,
    grade          TEXT,
    collection     TEXT,
    product_name   TEXT,
...
CREATE INDEX IF NOT EXISTS idx_pp_active
    ON price_catalog.product_prices(factory_id, version_id);
CREATE INDEX IF NOT EXISTS idx_pp_code
    ON price_catalog.product_prices(product_code);
CREATE INDEX IF NOT EXISTS idx_pp_search
    ON price_catalog.product_prices
...
-- ── 5. price_catalog.product_price_staging ───────────────────────────────────
CREATE TABLE IF NOT EXISTS price_catalog.product_price_staging (
    price_id         BIGSERIAL PRIMARY KEY,
    factory_id       BIGINT,
    version_id       BIGINT,
    product_code     TEXT,
    ...
    import_error     TEXT,
    CONSTRAINT stg_price_unit_check
        CHECK (price_unit IN ('per_sqm','per_piece','per_box','per_linear_m','unknown'))
);

CREATE INDEX IF NOT EXISTS idx_stg_version
    ON price_catalog.product_price_staging(version_id);
CREATE INDEX IF NOT EXISTS idx_stg_session
    ON price_catalog.product_price_staging(import_session_id);

-- ── 6. Seed factories (idempotent) ───────────────────────────────────────────
INSERT INTO price_catalog.factories (name, country, default_currency) VALUES
    ('Panaria', 'IT', 'EUR'),
    ('LEA',     'IT', 'EUR'),
    ('Padana',  'IT', 'EUR'),
    ('CDE',     'IT', 'EUR'),
...
-- ── 7. Seed import_profiles (idempotent) ─────────────────────────────────────
INSERT INTO price_catalog.import_profiles (factory_id, config)
...
ON CONFLICT ON CONSTRAINT uq_profile_factory DO NOTHING;

-- ── 8. Move customer/contact/project to customers schema (idempotent) ─────────
DO $$ BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema='sales' AND table_name='customer') THEN
    ALTER TABLE sales.customer SET SCHEMA customers;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema='sales' AND table_name='contact') THEN
    ALTER TABLE sales.contact SET SCHEMA customers;
  END IF;
  IF EXISTS (SELECT 1 FROM information_schema.tables
             WHERE table_schema='sales' AND table_name='project') THEN
