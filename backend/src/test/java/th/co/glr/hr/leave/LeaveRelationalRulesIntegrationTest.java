...
class LeaveRelationalRulesIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    void sickLeaveIsUnaffectedByAResignationRow() {
        ...
        // leave" once a resignation exists. Uses an attachment-bearing request shape is unnecessary
        // here -- AUTO_REJECTED on the (unrelated) missing-certificate gate is fine; the point is that
        // the systemNote must NOT mention resignation.
    ...
    void departmentCoverageBlocksTheLastRemainingEmployeeAndAllowsTheSamePairOnAFreeDay() {
        // Both sides on ONE fixture (same requester, same colleague, same department) -- the
        // vacuous-fixture trap this rule is explicitly at risk of: a "blocked" assertion alone would
        // pass even if the colleague-lookup silently found nobody. First call is blocked (colleague
        // has a PENDING request on the SAME day); second call, one week later where the colleague is
        // free, is allowed.
        ...
        long colleague = insertEmployee("COVER-EMP-002", LocalDate.parse("2015-01-01"), departmentId);
        seedActiveRequest(colleague, "VACATION", "2026-07-13", "2026-07-13", LeaveStatus.SUBMITTED);
    ...
    void aOnePersonDepartmentIsExemptFromTheCoverageRule() {
        ...
        // could never take any leave at all.
