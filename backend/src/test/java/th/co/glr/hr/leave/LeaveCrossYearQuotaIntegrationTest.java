...
class LeaveCrossYearQuotaIntegrationTest extends AbstractPostgresIntegrationTest {
    ...
    void backfillSqlReproducesTheParentRowsFiguresExactlyForALegacySingleYearRow() {
        ...
        jdbc.update("""
            ...
            -- §5.3.5 pool split (V161): mirrors what V161's own backfill computes for a legacy row
            -- whose employee has no hr.leave_carryover grant -- the whole consumed amount
            -- (quota_remaining_before - quota_remaining_after) lands on own_quota_days, carry-in
            -- stays 0. Keeps chk_lrqy_pool_matches_consumed satisfied without changing what this
            -- test is actually asserting (that V118's parent->child copy reproduces the parent's
            -- own figures exactly).
            SELECT leave_request_id, quota_year, total_days, paid_days, unpaid_days,
                   quota_remaining_before, quota_remaining_after,
                   0.00, GREATEST(quota_remaining_before - quota_remaining_after, 0)
